...
### Words this package uses

- Say “program” for an ALGAL organism. Use “organism” once where a reader needs the ALGAL term, and link ALGAL.
- A receipt is a record of a run that `verify` replays offline. Do not describe a recorded or fixture run as a live result, and give a live measurement its date and scope.
- Describe recovery by what the program does: it checks the plan first, retries once where Ghostget allows it, keeps going with the other rows, and names an escalation. Do not call it self-healing.
